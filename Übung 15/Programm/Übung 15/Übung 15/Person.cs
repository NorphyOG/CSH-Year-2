namespace Übung_15;
    ...
    public class Person
    {
        ...
        {
            string output = "";
            foreach (var vPerson in KontaktlistPersons)
            {
                output += vPerson.Value + "/" + vPerson.Key + " | ";
            }
            
            return $"{nameof(Vorname)}: {Vorname}, {nameof(Nachname)}: {Nachname}, {nameof(Adresse)}: {Adresse}, {nameof(KontaktlistPersons)}: {output}";
        }
    }
